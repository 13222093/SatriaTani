...
- **Map:** All real geographic tiles via Leaflet + react-leaflet. National `JavaMap` (Pusat Nasional) uses CARTO Dark Matter tiles with 12 hotspots placed at real regency centroids (Klaten, Demak, Banyuwangi, Indramayu, Jember, Karawang, Cirebon, Bantul, Pekalongan, Madiun, Sukabumi, Tegal); the four LAYER toggles (Risiko aktif / Loss ratio / Densitas polis / Anomali iklim) swap the map's data overlay. Parcel-scale `ParcelMap` (Verifier Console) uses Esri World Imagery satellite tiles over the real Sidorejo, Klaten bounding box. Cross-village `VillageMap` uses OSM street tiles with real Trucuk/Cawas/Bayat coordinates and a 25 km audit radius. MapLibre vector tiles still on roadmap.
