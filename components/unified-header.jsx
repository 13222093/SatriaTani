/* Unified header system — locked across Pusat, Verifier, Koperasi
   Pattern: ROW1 identity+breadcrumb+status+user · ROW2 section nav · ROW3 ⌘K search */
...
function StatusPill() {
  return (
    <div style={{
      ...
      <span style={{ width: 6, height: 6, borderRadius: '50%', background: EM, boxShadow: `0 0 0 3px ${EM}22` }} />
...
function UserChip({ name, org, role, initials, accent = NAVY }) {
  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: 10,
      paddingLeft: 14, borderLeft: '1px solid var(--line-navy)',
    }}>
      <div style={{ textAlign: 'right', lineHeight: 1.25 }}>
        <div style={{ fontSize: 12.5, fontWeight: 600, color: NAVY, fontFamily: 'var(--f-sans)' }}>{name}</div>
        <div style={{ fontSize: 10.5, color: 'var(--muted)', fontFamily: 'var(--f-sans)' }}>
          {org} · {role}
        </div>
      </div>
      <div style={{
...
/* The full assembly */
function UnifiedHeader({ scope, navItems, activeNav, user, accent }) {
  return (
    <div style={{ flexShrink: 0, background: '#fff', borderBottom: '1px solid var(--line-navy)' }}>
      {/* ROW 1 — identity */}
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ lineHeight: 1.2 }}>
            <div style={{
              ...
              fontWeight: 500, letterSpacing: '-0.01em', color: NAVY,
            }}>Satria Tani</div>
            <div style={{
              fontFamily: 'var(--f-mono)', fontSize: 9,
              letterSpacing: '0.18em', textTransform: 'uppercase',
              color: 'var(--muted)',
            }}>Protokol Asuransi Padi</div>
          </div>
        </div>
      ...
      {/* ROW 2 — section nav */}
      <SectionNav items={navItems} active={activeNav} />
...
Object.assign(window, { UnifiedHeader, MiniLogoMark });
